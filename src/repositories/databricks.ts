...
export type DatabricksEnrollment = {
  id: string;
  userId: string;
  cohortId: string;
...
export type DatabricksAttemptRow = {
  id: string;
  activityId: string;
  dayNumber: number;
...
export async function getDatabricksCohort(): Promise<{
  id: string;
  status: CohortStatus;
  timezone: string;
} | null> {
  const row = await prisma.cohort.findUnique({
    where: { slug: DATABRICKS_COHORT_SLUG },
    select: { id: true, status: true, timezone: true },
  });
  return row;
}

export async function findDatabricksEnrollment(
  userId: string,
): Promise<DatabricksEnrollment | null> {
...
export async function createDatabricksEnrollmentRow(
...
export async function listDatabricksProgress(
  enrollmentId: string,
  ...
  const rows = await prisma.activityAttempt.findMany({
    where: {
      enrollmentId,
      passed: true,
      activity: { id: { startsWith: "act_dbx_day_" } },
    },
...
export async function listDatabricksAttemptsForDay(
  enrollmentId: string,
  dayNumber: number,
): Promise<DatabricksAttemptRow[]> {
  const rows = await prisma.activityAttempt.findMany({
    where: {
      enrollmentId,
      activity: {
        dayNumber,
        id: { startsWith: "act_dbx_day_" },
      },
    },
...
export async function countDatabricksAttemptsForDay(
  enrollmentId: string,
  dayNumber: number,
): Promise<number> {
...
export async function listRecentDatabricksAttempts(
  enrollmentId: string,
  take: number,
): Promise<DatabricksAttemptRow[]> {
  const rows = await prisma.activityAttempt.findMany({
  ...
  const out: DatabricksAttemptRow[] = [];
  for (const row of rows) {
    const dn = row.activity.dayNumber;
...
export async function hasPassedDatabricksActivity(
  enrollmentId: string,
  activityId: string,
): Promise<boolean> {
...
export async function getLatestDatabricksAttemptAt(
  enrollmentId: string,
  dayNumber: number,
): Promise<Date | null> {
  const row = await prisma.activityAttempt.findFirst({
...
export async function recordDatabricksAttempt(
  tx: Prisma.TransactionClient,
  input: {
    enrollmentId: string;
    activityId: string;
...
export async function markDatabricksEnrollmentCompleted(
  tx: Prisma.TransactionClient,
  enrollmentId: string,
): Promise<void> {
...
export async function getDatabricksEnrollmentProgress(enrollmentId: string): Promise<{
...
function consecutiveStreaks(
  ...
  if (set.has(todayKey) || set.has(yesterdayKey)) {
    let cursor = set.has(todayKey) ? todayKey : yesterdayKey;
...
export async function recomputeDatabricksProgress(
  tx: Prisma.TransactionClient,
  enrollmentId: string,
): Promise<void> {
  ...
  const activities = await tx.activity.findMany({
    where: { id: { startsWith: "act_dbx_day_" } },
  ...
  const percentCompleteBp = Math.round(
    (completedActivities / DATABRICKS_TOTAL_DAYS) * 10000,
  );
  const now = new Date();
